    ...
    <>
      <Card className="overflow-hidden group" hover>
        ...
        <div className="p-4">
          ...
          <p className="text-xs text-[#C9A84C] font-medium mb-1">{cert.issuingBody}</p>
          {cert.issueDate && (
            <p className="text-xs text-[#718096] mb-1">Issued: {cert.issueDate}</p>
          )}
          {cert.certId && (
            <p className="text-xs text-[#A0AEC0] mb-3 font-mono">{cert.certId}</p>
          )}
          {cert.description && (
